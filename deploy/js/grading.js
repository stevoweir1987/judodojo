...
function openGradingReadiness() {
  const overlay = document.getElementById('grading-overlay');
  overlay.innerHTML = buildGradingReport();
...
// ── REPORT BUILDER ───────────────────────────────────────────────
function buildGradingReport() {
  const belt = getActiveBeltForGrading();
        ...
        ${belt.groups.map(g => {
          ...
          ${groupMissing.map(item => {
            ...
            <div class="gr-missing-item">
              ...
              ${vid ? `<button class="gr-watch-btn" onclick="event.stopPropagation();openModal('${esc(item)}')">&#9654;</button>` : ''}
            </div>`;
          }).join('')}`;
        }).join('')}
      </div>
